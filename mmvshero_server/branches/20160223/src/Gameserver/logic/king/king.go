...
func (this *King) KingAddLv() common.RetCode {
	...
	if !this.owner.IsEnoughBlood(this.scheme_data.NeedBlood) || !this.owner.IsEnoughSoul(this.scheme_data.NeedSoul) {
		LogError("add lv fail!:", this.scheme_data.NeedBlood, this.scheme_data.NeedSoul)
		return common.RetCode_Unable
	}
...
func (this *King) KingAddLvGm(addLv int32) common.RetCode {
	...
	for ; oldKingLv <= newKingLv; oldKingLv++ {
	...
	this.KingCache.SetKingLv(newKingLv)
	this.owner.HeroFreshLv(this.KingCache.GetKingLv())
	this.owner.SoldierFreshLv(this.KingCache.GetKingLv())
	this.owner.MapExpand(this.GetMapSize())
...
func (this *King) KingSkillStartLvUp(skillId int32, usedCoin bool) common.RetCode {
	...
	//魔血魔魂
	if !this.owner.IsEnoughBlood(skillLvUp.LvUpBlood) || !this.owner.IsEnoughSoul(skillLvUp.LvUpSoul) {
		return common.RetCode_Unable
	}

	if queueSkillId > 0 || this.owner.BuildingGetQueueUid() > 0 {
		LogDebug("queueSkillId > 0 || this.owner.BuildingGetQueueUid() > 0 : ", queueSkillId, this.owner.BuildingGetQueueUid())
