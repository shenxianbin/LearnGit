...
func (this *PvpSys) PvpQuery() (int32, common.RetCode) {
	if this.is_query_cd() {
		return -1, common.RetCode_CoolDown
	}
	...
	if err != nil {
		LogError(err)
		return -1, common.RetCode_Fail
	}

	rank, err := resp.Int()
	if err != nil {
		LogError(err)
		return -1, common.RetCode_Unable
	}
...
func (this *PvpSys) PvpMatch() *protocol.MsgPvpMatchRet {
	if len(this.PvpCache.FightHeroList) == 0 {
		...
		return &protocol.MsgPvpMatchRet{
			Retcode: proto.Int32(int32(common.RetCode_Fail)),
		}
	}
	...
	if owner_role_state.IsFightLock() {
		...
		return &protocol.MsgPvpMatchRet{
			Retcode: proto.Int32(int32(common.RetCode_Unable)),
		}
	}

	if !this.owner.IsEnoughBlood(this.owner.GetPvpSearchNeedBlood()) || !this.owner.IsEnoughSpirit(scheme.Commonmap[define.PvpCostSpirit].Value) {
		LogDebug("PvpMatch IsNotEnoughResource AtkUid : ", this.owner.GetUid())
		return &protocol.MsgPvpMatchRet{
			Retcode: proto.Int32(int32(common.RetCode_Unable)),
		}
	}
	...
	for i := 0; i < pvpmatch_limit; i++ {
		...
		if resp_str != nil && len(resp_str) > 0 {
			...
			for j := 0; j < pvpmatch_limit; j++ {
				...
				this.owner.CostBlood(this.owner.GetPvpSearchNeedBlood(), true, true)
