...
const (
	...
	sql_heros_insert = "INSERT INTO heros SET playerId=?,uid=?,schemeId=?,level=?,stage=?,rank=?"
	sql_heros_update = "UPDATE heros SET level=?,stage=?,rank=? WHERE playerId=? AND uid=? AND schemeId=?"
...
func init_protocol() {
	...
	RegisterMsg(int32(static.MsgStaticCode_Hero), func(msg []byte) {
		...
		mysql.AsynExec(func(query string, args ...interface{}) {
			...
			if affect, _ := res.RowsAffected(); affect <= 0 {
				res, err := mysql.Exec(sql_heros_insert, req.GetRoleUid(), req.GetUid(), req.GetSchemeId(), req.GetLv(), req.GetStage(), req.GetRank())
				if err != nil {
					LogError(err)
					return
				}
		...
		}, sql_heros_update, req.GetLv(), req.GetStage(), req.GetRank(), req.GetRoleUid(), req.GetUid(), req.GetSchemeId())
	})

	RegisterMsg(int32(static.MsgStaticCode_HeroDel), func(msg []byte) {
		req := &static.MsgStaticHeroDel{}
		err := proto.Unmarshal(msg, req)
		if err != nil {
			LogError(err)
			return
		...
		LogDebug("MsgStaticCode_HeroDel : ", req)
		...
		}, sql_heros_del, req.GetRoleUid(), req.GetUid())
	})
