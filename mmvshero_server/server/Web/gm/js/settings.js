...
$(function() {
    ...
    e = $("body"),
    f = function() {
        a.data("bs.popover").hoverState = "out",
        a.popover("hide")
    },
    g = function(b) {
    ...
    k = function(a, b) {
        b = void 0 == b ? !0 : !1,
        1 == a ? e.removeClass("sidebar-hidden") : e.addClass("sidebar-hidden"),
        b && triggerChartsResize()
    };
    h(c.sidebar),
    i(c.background),
    j(c.sidebarState, !1),
    k(c.displaySidebar, !1),
    a[0] && (a.popover({
        ...
        return $(".page-header .dropdown.open .dropdown-toggle").dropdown("toggle"),
        //$(document).on("click", g),
        $(this).focus(),
        !1
    ...
    }), d.on("click", ".popover #sidebar-toggle .btn",
    function() {
        var a = $(this),
        b = a.data("value");
        h(b),
        c.sidebar = b,
        localStorage.setItem("settings-state", JSON.stringify(c))
    }), d.on("click", ".popover #background-toggle .btn",
    function() {
        var a = $(this),
        b = a.data("value");
        i(b),
        c.background = b,
        localStorage.setItem("settings-state", JSON.stringify(c))
    }), d.on("click", ".popover #display-sidebar-toggle .btn",
    function() {
        var a = $(this),
        b = a.data("value");
        k(b),
    ...
    function() {
        var a = $(this),
        b = a.data("value");
        "icons" == b && closeNavigation(),
        j(b),
        c.sidebarState = b,
        localStorage.setItem("settings-state", JSON.stringify(c))
    }), ($("#sidebar").is(".sidebar-icons") || $(window).width() < 1049) && $(window).width() > 767 && closeNavigation(), d.on("click", ".popover [data-toggle='buttons-radio'] .btn:not(.active)",
    function() {
        var a = $(this),
        b = a.parent().find(".btn");
...
window.LightBlue = {
    ...
    changeColor: function(a, b, c) {
        ...
        h = f ? [f[1], f[2], f[3]] : a.replace(/^#?([a-f0-9][a-f0-9])([a-f0-9][a-f0-9])([a-f0-9][a-f0-9])/i,
        function() {
