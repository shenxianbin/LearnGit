...
func notifyGmCommand(roleUid int64) error {
	LogDebug("notifyGmCommand Gameserver : ", " role_uid: ", roleUid)
	...
	msg.SetContent(buf)
	client.ClientManager().SendToGameServer(msg)
	return nil
}

func notifyLantern(serverId int32, typ int32, id int32, time int64, content []byte) error {
	LogDebug("notifyLantern server: ", serverId, " type: ", typ, " id: ", id, " time: ", time, " content: ", content)
	msg := packet.NewPacket(0, int32(gm.GmMsgCode_GmLanternNotify))
	pack := new(gm.MsgGmLanternNotify)
	pack.SetType(typ)
	pack.SetId(id)
	pack.SetTime(time)
...
func handlerGmLantern(w http.ResponseWriter, r *http.Request) {
	...
	if typStr == "" || idStr == "" || timeStr == "" || content == "" {
		fmt.Fprintf(w, retNull, "Args Empty", "")
		return
	}

	typ, _ := strconv.ParseInt(typStr, 10, 32)
	id, _ := strconv.ParseInt(idStr, 10, 32)
	time, _ := strconv.ParseInt(timeStr, 10, 64)
	serverId, _ := strconv.ParseInt(serverIdStr, 10, 32)

	notifyLantern(int32(serverId), int32(typ), int32(id), time, []byte(content))

	fmt.Fprintf(w, retNull, "Lantern Success", "")
}
...
func GMWebInit(addr string) {
	...
	http.HandleFunc("/gmlantern", handlerGmLantern)
