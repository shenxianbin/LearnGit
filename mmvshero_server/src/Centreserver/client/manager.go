...
func (this *clientM) SendToGameServer(packet *packet.GxPacket) {
	if c, has := this.serverClientMap["GameServer"][gameserver]; has {
		c.SendPacket(packet)
		LogDebug("send to GameServer ", gameserver)
	}
}
...
func (this *clientM) Init() {
	...
	GxService().RegisterMsg("GameServer", define.MSGCODE_HELLO_REQ, func(s *tcp.Session, p *packet.GxPacket) {
		...
		gameserver = int(msg.GetServerId())
	})
}
