...
func (this *clientM) Add(sid int64, client *Client) {
	...
	galaxy.LogDebug("SID [ ", sid, " ] client added and now role num : ", len(this.clientMap))
}

func (this *clientM) Remove(sid int64) {
	delete(this.clientMap, sid)
	galaxy.LogDebug("SID [ ", sid, " ] client removed and now role num : ", len(this.clientMap))

}

func (this *clientM) RemoveAll() {
	this.clientMap = make(map[int64]*Client)
	if _, has := this.serverClientMap["Gameserver"]; has {
		this.serverClientMap["Gameserver"] = make(map[int]*ServerClient)
	}
	galaxy.LogDebug("start clear client")
	galaxy.GxService().RemoveAllListenerSession("Client")
...
func (this *clientM) BroadCastClient(packet *packet.GxPacket) {
