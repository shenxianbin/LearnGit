...
func GenArenaCacheKey(role_uid int64) string {
	return fmt.Sprintf(cache_arena_key_t, role_uid)
}
...
func parse_score_lv_to_sortvalue(score int32, lv int32) int64 {
	return int64(score)*100 + int64(lv)
}

func parse_sortvalue_to_score_lv(sort_value int64) (score int32, lv int32) {
	return int32(sort_value / 100), int32(sort_value % 100)
}
...
func (this *ArenaSys) Load() error {
	...
	if this.ArenaCache.ShopRecords == nil {
		this.ArenaCache.ShopRecords = make(map[int32]int32)
	}
...
func (this *ArenaSys) ArenaQuery() (ret *protocol.MsgArenaQueryRet) {
	...
	if resp.IsNil() {
		my_rank = 0
		LogDebug("MyRank Is Nil : Uid : ", this.owner.GetUid())
	} else {
...
func (this *ArenaSys) ArenaFight(score int32, is_cost_order bool) (ret *protocol.MsgArenaFightRet) {
	...
	if this.GetBestScore() < scheme.ArenaBossmap[arena_manager.GetBossId()].Line1 && score > scheme.ArenaBossmap[arena_manager.GetBossId()].Line1 {
		award.AwardByInfo(this.parseAwardInfo(arena_manager.GetBossAward1()), this.owner, true)
	}

	if this.GetBestScore() < scheme.ArenaBossmap[arena_manager.GetBossId()].Line2 && score > scheme.ArenaBossmap[arena_manager.GetBossId()].Line2 {
		award.AwardByInfo(this.parseAwardInfo(arena_manager.GetBossAward2()), this.owner, true)
	}

	if this.GetBestScore() < scheme.ArenaBossmap[arena_manager.GetBossId()].Line3 && score > scheme.ArenaBossmap[arena_manager.GetBossId()].Line3 {
		award.AwardByInfo(this.parseAwardInfo(arena_manager.GetBossAward3()), this.owner, true)
	}

	this.SetBestScore(score)

	//添加成就
	this.owner.AchievementAddNum(14, score, false)
...
func (this *ArenaSys) ArenaShopBuy(pos int32) (ret *protocol.MsgArenaShopBuyRet) {
	...
	LogDebug(shop_scheme)
	if !this.IsEnoughArenaPoint(shop_scheme.CostPoint) {
	...
	if shop_scheme.ExchangeTimes > 0 {
		...
		if has {
			if shop_record >= shop_scheme.ExchangeTimes {
				...
				LogDebug("ShopRecord Add Pos : ", pos, " Count : ", this.ArenaCache.ShopRecords[pos])
			}
		} else {
			this.ArenaCache.ShopRecords[pos] = 1
			ret.SetCount(this.ArenaCache.ShopRecords[pos])
			LogDebug("ShopRecord New Pos : ", pos, " Count : ", this.ArenaCache.ShopRecords[pos])
		}
	}
	this.CostArenaPoint(shop_scheme.CostPoint, true, true)
	award.AwardByInfo(this.parseAwardInfo(arena_manager.GetShopAward()), this.owner, true)
	this.Save()
