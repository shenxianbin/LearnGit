...
func (this *PlunderTeam) IsFinish() bool {
	...
	if this.GetStartTime()+int64(scheme_data.EscortTime*3600) <= time.Now().Unix() {
		// if this.GetStartTime()+int64(60) <= time.Now().Unix() {
		LogDebug("true")
		return true
	}
	LogDebug("false")
...
func NewPlunderTeam(pos int32, role IRole, heros []int32, properties []int32, plunder_award_id int32) (*PlunderTeam, error) {
	...
	cur_properties := make([]int32, 6)
	for index, id := range properties {
		cur_properties[index] = id
	}
	for index, hero_id := range heros {
		...
		plunder_hero.SetProperties(cur_properties[index*2 : index*2+2])
...
func genPlunderAward(id int32, team *PlunderTeam, role IRole) error {
	...
	//战力
	var fight_value float32
	for _, v := range team.PlunderTeamCache.Heros {
		if hero_scheme, has := scheme.Heromap[v.GetSchemeId()]; !has {
			return errors.New(fmt.Sprintf("Hero Scheme Null : id : ", v.GetSchemeId()))
		} else {
			hero_rank_scheme := scheme.HeroRankGet(hero_scheme.MagicHeroRankId, v.GetRank())
			if hero_rank_scheme == nil {
				return errors.New(fmt.Sprintf("HeroRank Scheme Null : plan : ", hero_scheme.MagicHeroRankId, " rank : ", v.GetRank()))
			} else {
				LogDebug("lv : ", v.GetLv())
				LogDebug("rankParm : ", hero_rank_scheme.RankParm)
				fight_value += float32(float32(v.GetLv()) * float32(hero_rank_scheme.RankParm) / 100.0)
			}
		}
	}
	LogDebug("fight_value : ", fight_value)
	fight_value = fight_value / 3.0 / float32(role.GetLv())
	LogDebug("fight_value : ", fight_value)

	total_plunder_award := make([]*PlunderAwardCache, 0)

	//魔魂
	total_soul := int32(RandFloat(float64(scheme_ex_data.SoulAward[0]), float64(scheme_ex_data.SoulAward[1]))*float64(fight_value)*float64(role.GetSoulHour())/100.0 + 0.5)
	LogDebug("total_soul : ", total_soul)
	...
	if len(owner_soldier) > 0 {
		for i := 0; i < int(scheme_ex_data.SoldierAward.Num); i++ {
			...
			chip_num := int32(RandFloat(float64(scheme_ex_data.SoldierAward.Flow[0]), float64(scheme_ex_data.SoldierAward.Flow[1]))*float64(fight_value)*float64(role.GetSoldierChipHour())/100.0 + 0.5)
			LogDebug("soldier chip_num : ", chip_num)
	...
	//魔使
	if len(scheme_ex_data.HeroPool) > 0 {
		for i := 0; i < int(scheme_ex_data.HeroAward.Num); i++ {
			...
			chip_num := int32(RandFloat(float64(scheme_ex_data.HeroAward.Flow[0]), float64(scheme_ex_data.HeroAward.Flow[1]))*float64(fight_value)*float64(role.GetHeroChipHour())/100.0 + 0.5)
			LogDebug("hero chip_num : ", chip_num)
			for i, v := range total_plunder_award {
				if v.GetCode() == chip_id {
					total_plunder_award[i].SetAmount(total_plunder_award[i].GetAmount() + chip_num)
					find = true
...
func (this *PlunderSys) PlunderGuard(pos int32, heros []int32, properties []int32, plunder_award_id int32) (*protocol.PlunderTeam, common.RetCode) {
	...
	endTime := team.GetStartTime() + int64(scheme.PlunderAwardmap[team.GetPlunderId()].EscortTime*3600-scheme.Commonmap[define.PlunderBattleLockTime].Value)

	this.EnrollPool(this.owner.GetUid(), team.GetTeamId(), int64(totalExp), endTime)

	//添加成就
	this.owner.AchievementAddNum(11, 1, false)
