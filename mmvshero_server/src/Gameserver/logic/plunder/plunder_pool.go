...
func (this *Pool) getByScoreRange(totalExp, attackerId int64) *Node {
	...
	var i int64 = 0
	nodesLen := len(nodes)
	for times := 0; times < nodesLen; times++ {
		if times != 0 {
		...
		if temp != nil {
			if n, ok := temp.Value.(*Node); ok {
				if n.GetRoleUid() == attackerId {
					LogDebug("n.GetRoleUid() == attackerId:", n)
					unlock(temp.Key())
					continue
				}

				if Time() >= n.GetEndTime() {
					LogDebug("Time() >= n.GetEndTime():", n)
					go this.remove(n.GetRoleUid(), n.GetTeamId())
					unlock(temp.Key())
					continue
				}
...
func (this *Pool) getBattleLock(roleUid, teamId int64) int64 {
	key := fmt.Sprintf("%v:%v", roleUid, teamId)
	LogDebug("getBattleLock:key:", key)
	LogDebug("allLockers:", allLockers)
	lock(key)
	defer unlock(key)
