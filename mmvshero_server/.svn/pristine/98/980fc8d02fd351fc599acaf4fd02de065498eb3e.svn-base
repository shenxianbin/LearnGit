...
func handlerGmCommandQuery(w http.ResponseWriter, r *http.Request) {
	...
	kingCache := new(cache.KingCache)
	itemListCache := make(map[int64]*cache.ItemCache)
	heroListCache := make(map[int64]*cache.HeroCache)
	...
	//king
	resp, err = GxService().Redis().Cmd("GET", king.GenKingCacheKey(roleUid))
	if err != nil {
		fmt.Fprintf(w, retNull, "RoleKingInfo QueryRedis Error : "+err.Error(), "")
		return
	}

	if buf, _ := resp.Bytes(); buf != nil {
		err = proto.Unmarshal(buf, kingCache)
		if err != nil {
			fmt.Fprintf(w, retNull, "RoleKingInfo Parse ProtoBuf Error : "+err.Error(), "")
			return
		}
	}
	...
	for _, key := range cacheKeys {
		...
		if buf, _ := resp.Bytes(); buf != nil {
			...
			heroListCache[heroCache.GetUid()] = heroCache
		}
	}
	...
	resp, err = GxService().Redis().Cmd("SMEMBERS", soldier.GenSoldierInMapListKey(roleUid))
	if err != nil {
		fmt.Fprintf(w, retNull, "RoleSoldiersMapInfo QueryRedis Error : "+err.Error(), "")
		return
	}

	cacheKeys, err = resp.List()
	if err != nil {
		fmt.Fprintf(w, retNull, "RoleSoldiersMapInfo Parse List Error : "+err.Error(), "")
		return
	}
	...
	for _, key := range cacheKeys {
		...
		if buf, _ := resp.Bytes(); buf != nil {
			...
			if _, has := soldierListCache[soldierCache.GetSchemeId()]; has {
				soldierListCache[soldierCache.GetSchemeId()].SetNum(soldierListCache[soldierCache.GetSchemeId()].GetNum() + soldierCache.GetNum())
			} else {
				soldierListCache[soldierCache.GetSchemeId()] = soldierCache
			}
		}
