...
func RedisCmdLua(script, scriptSha1 string, args ...interface{}) *redis.Resp {
	resp, err := RedisCmd("EVALSHA", scriptSha1, args)
	if err != nil {
		if fmt.Sprintf("%s", err) == "NOSCRIPT No matching script. Please use EVAL." {
			resp, _ = RedisCmd("EVAL", script, args)
		}
	}
	return resp
}

func TimeSpend(title string) func() {
	start := time.Now()
	return func() {
		galaxy.LogDebug("%v Time spend is :%v Nanoseconds", title, time.Since(start).Nanoseconds())
	}
}
